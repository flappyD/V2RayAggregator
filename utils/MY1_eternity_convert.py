...
Eterniy_base_file = './MY1_EternityBase'
Eterniy_file = './MY1_Eternity'
Eternity_yml_file = './MY1_Eternity.yml'
readme = './MY1_README.md'
log_file = './MY1_LogInfo.txt'

provider_path = './update/provider/'
update_path = './update/'

sub_list_json = './sub/MY1_sub_list.json'

config_file = './update/provider/MY1_config.yml'
config_global_file = './update/provider/MY1_config-global.yml'
...
def eternity_convert(file, config, output, provider_file_enabled=True):
    ...
    all_provider = subs_function.convert_sub(
        "https://raw.githubusercontent.com/flappyD/V2RayAggregator/master/Eternity_MY_base64.txt", 'clash', "http://0.0.0.0:25500", False, extra_options="&udp=false")
    ...
    print(f"remove lines with name issue")
    ...
    print(f"take a part from begining of all lines")
    num = 200
    ...
    if provider_file_enabled:
        providers_files = {
            'all': provider_path + 'MY1_provider-all.yml',
